...
extern volatile uint8_t packetReceivedFlag;
...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&huart2, (uint8_t*)&rxBuffer[0], 1);
  ...
  while (1)
  {
    ...
    uint8_t rxBuffer[6] = {0}; // 5 chars + null terminator

    // Receive 5 characters (blocking)
    HAL_UART_Receive(&huart2, rxBuffer, 5, HAL_MAX_DELAY);
